...
import { NotFoundError, ValidationError, StorageError, CryptoError, errorToResponse, logError } from '../../utils/errors.js';
...
export async function handleExportBackup(request, env, backupKey) {
	...
	try {
		...
		// 从 R2 获取备份（备份存储在 backups/ 前缀下）
		const r2Key = backupKey.startsWith('backups/') ? backupKey : `backups/${backupKey}`;
		const r2Obj = await env.BACKUP_R2.get(r2Key);

		if (!r2Obj) {
			return createErrorResponse('备份不存在', `备份文件 "${backupKey}" 不存在`, 404, request);
		}

		const backupContent = await r2Obj.text();
