...
import { handleGetR2Backups, handleRestoreR2Backup } from '../api/r2-backup.js';
	...
	try {
		...
		if (requiresAuth(pathname)) {
			...
			if (!authDetails || !authDetails.valid) {
				...
				// 检查是否未设置密码
				const pwObj = await env.BACKUP_R2.get('auth/user_password');
				const storedPasswordHash = pwObj ? await pwObj.text() : null;
...
async function handleApiRequest(pathname, method, request, env) {
	...
	if (pathname === '/api/r2-backup') {
		if (method === 'GET') {
			return handleGetR2Backups(request, env);
		}
		return createErrorResponse('方法不允许', `不支持的HTTP方法: ${method}`, 405, request);
	}

	// R2 备份还原 API
	if (pathname === '/api/r2-backup/restore') {
		if (method === 'POST') {
			return handleRestoreR2Backup(request, env);
		}
		return createErrorResponse('方法不允许', `不支持的HTTP方法: ${method}`, 405, request);
	}
